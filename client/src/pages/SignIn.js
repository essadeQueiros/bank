...
const SignIn = (props) => {
    ...
    const checkInputs = () => {
        if(firstName !== '' && lastName !== '' && verifyEmail() && verifyPassword()){
            return true
        }
        return false
    }
        ...
        <div className="SignIn w-screen h-screen bg-gray-800 pt-60">
            <div className="transition-all bg-gray-900 py-2 px-2 ease-in-out duration-300 w-5/6 mx-auto 2xl:w-1/4 xl:w-1/3 lg:w-2/5 md:w-1/2 flex flex-col items-center justify-center border-2 border-black">
                <div className='w-full py-5 border-white border-2'>
                    ...
                    {openSignUp && <button onClick={() => {signUp()}} className="mt-4 w-40 h-10 rounded text-lg text-white border-2 border-white bg-blue-600 !disabled:hover:bg-transparent !disabled:hover:border-2 !disabled:hover:border-blue-600 disabled:opacity-50">Sing Up</button>}
